 ...
 *
 * There is no backend. The day's board is *derived* from the date through a seeded generator,
 * so every device produces the same starting position for the same day without anything being
 * fetched, and a past day can be replayed exactly. That is what makes the paywall's "full
 * daily-challenge archive" a real feature rather than a promise of content that would have to
 * be shipped later.
 *
 * Pure and dependency-free: the date is always passed in, never read from the clock here.
 */
import { type Grid, emptyGrid, spawn } from "./board";
...
/** The board everyone gets for a given day. */
export function startingGrid(key: string): Grid {
  const rng = seededRng(dailySeed(key));
